...
import { Wrapper, SportsReads, SectionHeader } from './shared'
...
const Sports = () => (
  <StaticQuery
    query={graphql`
      query {
        SquareArticles: allFile(
          filter: { relativePath: { eq: "final-sports-2020.json" } }
        ) {
          edges {
            node {
    ...
    render={data => {
      ...
      const {
        node: { childrenFinalSports2020Json: articles },
      } = SquareArticles.edges[0]
      const {
        node: { childrenFinalSports2020Json: rectArticles },
      } = RectangleArticles.edges[0]
      // const {
        ...
        <Wrapper>
          <div className="row main" id="sports">
            ...
            <Row style={{ marginTop: '2rem' }}>
              <SportsPiece
                article={rectArticles[3]}
                style={{ paddingLeft: 0 }}
              />
              <SportsPiece
                article={rectArticles[4]}
